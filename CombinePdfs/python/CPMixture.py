...
class CPMixture(PhysicsModel):
    ...
    def doParametersOfInterest(self):
        ...
        self.modelBuilder.factory_('expr::a("cos(@0*{pi}/2)", alpha)'.format(pi=np.pi))
        self.modelBuilder.factory_('expr::b("sin(@0*{pi}/2)", alpha)'.format(pi=np.pi))
        ...
        # JHU samples have constant XS that does not depend on alpha. 
        # If the constrain_ggHYield option is set then add a scaling 
        # as a function of alpha -> XS = cos^2(alpha) + (3/2)^2*sin^2(alpha) to match MG5 XS
