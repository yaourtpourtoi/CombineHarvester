	    ...
	    if options.pullDef and nuis_p!=None: 
		    ...
		    if options.pullDef and nuis_p!=None:
		      ...
		      gr_fit_b.SetPointError(nuis_p_i-1,0,0,abs(nuis_x.getErrorLo()),nuis_x.getErrorHi())
	      	    ...
	      	    hist_fit_b.SetBinContent(nuis_p_i,0)
	      	    hist_fit_b.SetBinError(nuis_p_i,1)
	      	    hist_fit_b.GetXaxis().SetBinLabel(nuis_p_i,name)
	      	    gr_fit_b.GetXaxis().SetBinLabel(nuis_p_i,name)
	          if fit_name=='s':
		    if options.pullDef and nuis_p!=None:
		      #nx,ned,neu = CP.returnPullAsym(options.pullDef,nuis_x.getVal(),mean_p,nuis_x.getErrorHi(),sigma_pu,abs(nuis_x.getErrorLo()),abs(sigma_pd))
		      gr_fit_s.SetPoint(nuis_p_i-1, nx, nuis_p_i-0.5-0.1)
		      gr_fit_s.SetPointError(nuis_p_i-1, ned, neu, 0, 0)
		    else:
		      gr_fit_s.SetPoint(nuis_p_i-1, nuis_x.getVal(),  nuis_p_i-0.5-0.1)
		      gr_fit_s.SetPointError(nuis_p_i-1, abs(nuis_x.getErrorLo()), nuis_x.getErrorHi(), 0, 0)
	      	    ...
	      	    gr_fit_s.GetYaxis().SetBinLabel(nuis_p_i,name)
...
if options.plotfile:
    ...
    canvas_nuis = ROOT.TCanvas("nuisances", "nuisances", 1200, 800)
    ...
    hist_prefit.SetBarWidth(0.0)
    hist_prefit.GetXaxis().SetLabelSize(0.03)
    hist_prefit.SetStats(0)
    hist_prefit.Draw("E2 hbar")
    ...
    hist_fit_e_b.Draw("hbar")
    hist_fit_e_s.Draw("hbarsame")
